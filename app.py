...
def comparar_k(df, lc, oc, k_values, weight_type="sin_peso", kgc=None, pedc=None):
    """
    ...
    W=build_w(df[valid],weight_type,kgc,pedc)
    rows=[]
    for k in k_values:
        if k>=len(X): continue
...
with T[0]:
    ...
    if up is None:
        ...
        if df_l is not None and not df_l.empty:
            ...
            hc=bool(ltc and lnc); hcl=bool(clc)
            ...
            if st.button(btn_label,use_container_width=True,type="primary"):
                ...
                if hc:
                    # Renombrar coordenadas al canónico y marcar geocodificado
                ...
                if vn<5: st.error(f"Solo {vn} puntos. Necesitas ≥5.")
                    ...
                    if meth in ("KMeans","MiniBatchKMeans"):
                        ...
                        with st.expander("📊 Comparador de escenarios K — Método del Codo"):
                            ...
                            wt_codo=st.radio("Gravedad para comparación",list(WEIGHTS.keys()),
                                             format_func=lambda x:WEIGHTS[x],horizontal=True,key="codo_w")
                            kgs_c=peds_c=None
                            if wt_codo in ("kg","combinado") and nc:
                                di_c=nc.index(kgd) if kgd in nc else 0
                                kgs_c=st.selectbox("KG col",nc,index=di_c,key="codo_kg")
                            if wt_codo in ("pedidos","combinado") and nc:
                                di_c=nc.index(pedd) if pedd in nc else 0
                                peds_c=st.selectbox("Pedidos col",nc,index=di_c,key="codo_ped")
                            if st.button("▶️ Calcular comparación",use_container_width=True,key="codo_btn"):
                                with st.spinner(f"Evaluando K={k_list[0]}…{k_list[-1]}…"):
                                    df_codo=comparar_k(df_cl,lc3,oc3,k_list,wt_codo,kgs_c,peds_c)
...
with T[7]:
    ...
    if df_ex is None: st.warning("⚠️ No hay datos para exportar.")
        ...
        df_for_map=(ss.get("df_clustered") or ss.get("df_geocoded"))
